...
#include "modules/drawing/Visualizer_GLUT.h"
...
void _g_OnSpecialKeyPressed(int key, int x, int y) {
  _g_keySpecialStates[key] =
      true;  // Set the state of the current key to pressed
}

void _g_OnSpecialKeyUp(int key, int x, int y) {
  _g_keySpecialStates[key] =
      false;  // Set the state of the current key to not pressed
}
...
void Visualizer_GLUT::DrawBackground() {
  ...
  // draw background
  glBegin(GL_QUADS);
  glColor4f(_bgColorBottomLeft[0], _bgColorBottomLeft[1], _bgColorBottomLeft[2],
            1);
  glVertex2f(0, 0);
  glColor4f(_bgColorBottomRight[0], _bgColorBottomRight[1],
            _bgColorBottomRight[2], 1);
  glVertex2f(1, 0);
  // top
...
void Visualizer_GLUT::DrawCoordinateReference() {
  ...
  // V3D rosePos = pos + (lookat-pos).norm()*_refLoc.x + left*_refLoc.y + realUp
  // * _refLoc.z;
  V3D rosePos(-2, -2, -.5f);
...
SLR::LineD Visualizer_GLUT::ScreenToPickVector(double x, double y) {
  float winY = (float)(viewport[3] - y);  // Subtract The Current Mouse Y
                                          // Coordinate From The Screen Height.
  float winX = (float)(x);

  V3D ray;
  gluUnProject(winX, winY, .5, modelMatrix, projMatrix, viewport, &ray.x,
               &ray.y, &ray.z);

  return SLR::LineD(
      _camera.FilteredPos(),
      _camera.FilteredPos() + (ray - _camera.FilteredPos()).norm() * 50);
}

void Visualizer_GLUT::DrawTrajectories(shared_ptr<QuadDynamics> quad) {
  if (quad) {
    if (quad->controller && showRefTrajectory) {
      VisualizeTrajectory(quad->controller->trajectory, true, V3F(0, 1, 1), 1.f,
                          V3F(.1f, .2f, 1), quad->color,
...
void Visualizer_GLUT::Paint() {
  ...
  // retrieve modelview/projection/viewport info to calculate 2D coords from 3D
  // coords
...
void Visualizer_GLUT::DrawBottomStatus(float simTime) {
  ...
  sprintf_s(buf, 100, "%s t=%.3f%s", _scenarioName.c_str(), simTime,
            paused ? " Paused" : "");
...
void Visualizer_GLUT::VisualizeQuadCopter(shared_ptr<QuadDynamics> quad) {
  glLineWidth(1);
  glEnable(GL_LINE_SMOOTH);
  _glDraw->DrawQuadrotor2(quad->Position(), quad->Attitude(), quad->color,
                          V3F(quad->cx, quad->cy, 0), quad->M / 0.5f, quad->L);

  if (showPropCommands) {
    V3D pos = quad->Position();
    V3D fl = quad->GetArmLength() / sqrtf(2) *
             quad->Attitude().Rotate_BtoI(V3F(1, 1, 0));
    V3D fr = quad->GetArmLength() / sqrtf(2) *
             quad->Attitude().Rotate_BtoI(V3F(1, -1, 0));
...
void Visualizer_GLUT::SetKeyboardFunc(void (*callback)(unsigned char, int,
                                                       int)) {
  glutKeyboardFunc(callback);
}

void DrawXYGrid(V3D center, double lenX, double lenY, int numCellsX,
                int numCellsY) {
