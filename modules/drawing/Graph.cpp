...
#include "modules/drawing/Graph.h"
...
#include "modules/drawing/AbsThreshold.h"
#include "modules/drawing/WindowThreshold.h"
#include "modules/drawing/SigmaThreshold.h"
...
void Graph::AddAbsThreshold(string path) {
  ...
  shared_ptr<AbsThreshold> thr(new AbsThreshold(
      args[0], (float)atof(args[1].c_str()), (float)atof(args[2].c_str())));
...
void Graph::AddWindowThreshold(string path) {
  ...
  shared_ptr<WindowThreshold> thr(new WindowThreshold(
      args[0], (float)atof(args[1].c_str()), (float)atof(args[2].c_str())));
  _analyzers.push_back(thr);
}
...
void Graph::AddSigmaThreshold(string path) {
  ...
  shared_ptr<SigmaThreshold> thr(new SigmaThreshold(
      args[0], args[1], args[2], (float)atof(args[3].c_str()),
      (float)atof(args[4].c_str()), (float)atof(args[5].c_str())));
  _analyzers.push_back(thr);
}
...
// Given a data range (r), what is the format string we should use for printing
// the tick labels, how many ticks should there be, and what should the tick
// labels be? This is a rough pragmatic solution that usually produces
// reasonable results
//
// Input: low, high -> range of the data of interest
// Returns: string w/ printf format to use for printing the tick labels (e.g.
// "%.2lf) Optional returns:
...
void Graph::Draw() {
  ...
  string tickYFormat =
      GetValueFormat(lowY, highY, &tickYDelta, &tickYLow, &tickYHigh);
  ...
  glColor3f(.9f, .9f, .9f);
  for (float y = tickYLow; y <= tickYHigh; y += tickYDelta) {
    if (y < (lowY + (highY - lowY) * .05f) ||
        y > (highY - (highY - lowY) * .05f))
      continue;
    char buf[100];
    sprintf_s(buf, 100, tickYFormat.c_str(), y);
    DrawStrokeText(buf, lowX + .01f * (highX - lowX), y, 0, 1.2f,
                   (highX - lowX) / 3.f, (highY - lowY) / 3.f * 2.f);
  }
  ...
  for (unsigned int i = 0; i < _series.size(); i++) {
    ...
    DrawStrokeText_Align(ToLower(_series[i]._legend).c_str(), .95f,
                         .8f - j * .205f, 0, 1.5f, 1.f, 2.f, GLD_ALIGN_RIGHT);
    j++;
  }
  ...
  if (!_title.empty()) {
    glColor3f(1, 1, 1);
    DrawStrokeText_Align(_title.c_str(), 0.01f, .8f, 0, 1.5f, 1.f, 2.f,
                         GLD_ALIGN_CENTER);
  }
}
