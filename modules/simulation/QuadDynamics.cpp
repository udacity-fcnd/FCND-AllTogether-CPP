...
#include "modules/control/ControllerFactory.h"
#include "modules/planning/PlannerFactory.h"

#include "modules/estimation/QuadEstimatorEKF.h"
#include "modules/simulation/SimulatedGPS.h"
#include "modules/simulation/SimulatedIMU.h"
#include "modules/simulation/SimulatedMag.h"
...
QuadDynamics::QuadDynamics(string name) : BaseDynamics(name) {
  Initialize();
}
...
void QuadDynamics::ResetState(V3F pos, V3F vel, Quaternion<float> att,
                              V3F omega) {
  BaseDynamics::ResetState(pos, vel, att, omega);

  controllerUpdateInterval = (double)ONBOARD_TS;
  timeSinceLastControllerUpdate =
      controllerUpdateInterval;  // Force the simulation to start with a
                                 // controller update

  xyzDisturbance = rotDisturbance = V3D();
}

void QuadDynamics::SetPosVelAttOmega(V3F pos, V3F vel, Quaternion<float> att,
                                     V3F omega) {
  BaseDynamics::ResetState(pos, vel, att, omega);
}
...
int QuadDynamics::Initialize() {
  ...
  controller = CreateController(
      _name, config->Get(_name + ".ControlType", "QuadControl"),
      config->Get(_name + ".ControlConfig", "QuadControlParams"));
  if (controller) {
    controller->SetTrajTimeOffset(
        config->Get(_name + ".TrajectoryTimeOffset", 0));
    controller->SetTrajectoryOffset(
        config->Get(_name + ".TrajectoryOffset", V3F()));
  } else {
  ...
  planner = CreatePlanner(map, 10.0);
  controller->trajectory =
      planner->trajectory;  // copy trajectory to controller
  ...
  for (unsigned int i = 0; i < sensorList.size(); i++) {
    ...
    if (s == "SIMGPS") {
      shared_ptr<SimulatedGPS> simGPS(new SimulatedGPS(
          config->Get(_name + ".SimGPSConfig", "SimGPS"), _name));
      sensors.push_back(simGPS);
    } else if (s == "SIMIMU") {
      shared_ptr<SimulatedIMU> simIMU(new SimulatedIMU(
          config->Get(_name + ".SimIMUConfig", "SimIMU"), _name));
      sensors.push_back(simIMU);
    } else if (s == "SIMMAG") {
      shared_ptr<SimulatedMag> simMag(new SimulatedMag(
          config->Get(_name + ".SimIMUConfig", "SimMag"), _name));
...
void QuadDynamics::Run(float dt, float simulationTime, int& idum,
  ...
  while (remainingTimeToSimulate >
         0.000001)  // Time intervals lower than that are just discarded (for
                    // speed of running)
  {
    if (timeSinceLastControllerUpdate >= controllerUpdateInterval) {
      ...
      // This is the update of the onboard controller -- runs timeout logic,
      // sensor filtering, estimation, controller, and produces a new set of
      // motor commands
      if (controller && _useIdealEstimator) {
        controller->UpdateEstimates(Position(), Velocity(), quat, Omega());
      } else if (controller && estimator) {
        controller->UpdateEstimates(
            estimator->EstimatedPosition(), estimator->EstimatedVelocity(),
            estimator->EstimatedAttitude(), estimator->EstimatedOmega());
      }

      if (controller) {
        curCmd =
            controller->RunControl(controllerUpdateInterval, simulationTime);
    ...
    const double simStep =
        MIN(controllerUpdateInterval - timeSinceLastControllerUpdate,
            remainingTimeToSimulate);
    Dynamics(simStep, simulationTime, externalForceInGlobalFrame,
             externalMomentInBodyFrame, idum);
    timeSinceLastControllerUpdate += simStep;
    remainingTimeToSimulate -= simStep;
  }
}

void QuadDynamics::Dynamics(float dt, float simTime, V3F external_force,
  ...
  for (int i = 0; i < 4; i++) {
    curCmd.desiredThrustsN[i] =
        CONSTRAIN(curCmd.desiredThrustsN[i], minMotorThrust, maxMotorThrust);
    motorCmdsN(i) = curCmd.desiredThrustsN[i] +
                    randomMotorForceMag * ran1_inRange(-1.f, 1.f, idum);
  }
  ...
  for (int m = 0; m < 4; m++) {
    if (motorCmdsN(m) >= motorCmdsOld(m)) {
      motorCmdsN(m) =
          motorCmdsN(m) * ((float)dt / ((float)dt + (float)tauaUp)) +
          motorCmdsOld(m) * ((float)tauaUp / ((float)dt + (float)tauaUp));
    } else {
      motorCmdsN(m) =
          motorCmdsN(m) * ((float)dt / ((float)dt + (float)tauaDown)) +
          motorCmdsOld(m) * ((float)tauaDown / ((float)dt + (float)tauaDown));
    }
  }

  float total_thrust =
      motorCmdsN(0) + motorCmdsN(1) + motorCmdsN(2) + motorCmdsN(3);
  V3F oldPos;
  ...
  matrix::Vector<float, 3> total_moment =
      motor_matrix * motorCmdsN + ext_moment;
