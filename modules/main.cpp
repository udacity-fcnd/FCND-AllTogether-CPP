...
#include "modules/utility/Timer.h"
...
#include "modules/mavlinkNode/MavlinkTranslation.h"
...
void OnTimer(int) {
  ...
  if (receivedResetRequest == true ||
      (ToUpper(config->Get("Sim.RunMode", "Continuous")) == "REPEAT" &&
       endTime > 0 && simulationTime >= endTime)) {
    ResetSimulation();
  }
  ...
  if (lastDraw.ElapsedSeconds() > 0.030) {
    ...
    if (mlNode) {
      ...
      mlNode->Send(MakeMavlinkPacket_LocalPose(
          simulationTime, quads[0]->Position(), quads[0]->Velocity()));
      mlNode->Send(MakeMavlinkPacket_Attitude(
          simulationTime, quads[0]->Attitude(), quads[0]->Omega()));
    }
  }
...
vector<QuadcopterHandle> CreateVehicles() {
  ...
  while (1) {
    ...
    if (config->Exists(buf)) {
      QuadcopterHandle q =
          QuadDynamics::Create(config->Get(buf, "Quad"), (int)ret.size());
      ret.push_back(q);
    } else {
      break;
    }
